...
struct BlogViewControllerTests {
    ...
    @Test func getSpecificBlogPost() async throws {
        try await withApp { app in
            ...
            try await app.testing().test(.GET, "blog/\(try post.requireID())", afterResponse: { res async in
    ...
    @Test func getSpecificBlogPost_WhenPostDoesNotExist() async throws {
        try await withApp { app in
            try await app.testing().test(.GET, "blog/\(UUID())", afterResponse: { res async in
    ...
    @Test func newBlogPost() async throws {
        try await withApp { app in
            ...
            try await app.testing().test(.GET, "blog/new_post", beforeRequest: { req in
    ...
    @Test func newBlogPost_ReturnsUnauthorizedWithNoUser() async throws {
        try await withApp { app in
            ...
            try await app.testing().test(.GET, "blog/new_post", afterResponse: { res async in
    ...
    @Test func editBlogPost() async throws {
        try await withApp { app in
            ...
            try await app.testing().test(.GET, "blog/edit_post/\(post.requireID())", beforeRequest: { req in
               req.headers.add(name: .cookie, value: sessionCookie)
            }, afterResponse: { res async in
                do {
                    let csrfToken = try #require(app.sessions.memory.storage.sessions.first?.value["__VaporCSRFSessionKey"])
    ...
    @Test func editBlogPost_ReturnsNotFoundIfPostIDIsWrong() async throws {
        try await withApp { app in
            ...
            try await app.testing().test(.GET, "blog/edit_post/\(UUID())", beforeRequest: { req in
               req.headers.add(name: .cookie, value: sessionCookie)
            }, afterResponse: { res async in
                #expect(res.status == .notFound)
            })
    ...
    @Test func editBlogPost_ReturnsUnauthorizedWithNoUser() async throws {
        try await withApp { app in
            ...
            try await app.testing().test(.GET, "blog/edit_post/\(post.requireID())", afterResponse: { res async in
                #expect(res.status == .unauthorized)
            })
        }
    }
    ...
    @Test func publishNewPost_WithNewTags() async throws {
        ...
        try await withApp { app in
            ...
            try await app.testing().test(.POST, "blog/new_post/web_publish", beforeRequest: { req in
                req.headers.add(name: .cookie, value: sessionCookie)
    ...
    @Test func editPost_WithNewTags() async throws {
        struct EditPostRequest: Content {
            let post_id: String
            let post_title: String
            let post_tags: String
            let post_description: String
        ...
        try await withApp { app in
            ...
            let userQuery = try await User.find(UUID(uuidString: "6D50873A-D6EE-4B1E-9ADF-99067B2B4467"), on: app.db)
            let user = try #require(userQuery)
            let sessionCookie = try await user.createSession(app: app)
            
            let tagCount = try await Tag.query(on: app.db).all().count
            
            let postQuery = try await BlogPost.query(on: app.db).first()
            let post = try #require(postQuery)
            
            let request = try EditPostRequest(post_id: post.requireID().uuidString,
                                              post_title: UUID().uuidString,
                                              post_tags: "\(UUID().uuidString) \(UUID().uuidString)",
                                              post_description: UUID().uuidString,
                                              post_content: UUID().uuidString,
                                              csrfToken: [UInt8].random(count: 32).base64)
            ...
            try await app.testing().test(.POST, "blog/edit_post/web_update", beforeRequest: { req in
                req.headers.add(name: .cookie, value: sessionCookie)
                try req.content.encode(request)
            }, afterResponse: { res async in
                do {
                    ...
                    #expect(post.author?.email.mailbox == user.email.mailbox)
                    #expect(post.tags.count >= request.post_tags.components(separatedBy: " ").count)
                    #expect(post.tags.map(\.canonicalTitle).sorted().contains(request.post_tags.components(separatedBy: " ").map { $0.lowercased() }.sorted()))
                    let newTagCount = try await Tag.query(on: app.db).all().count
                    #expect(tagCount + 2 == newTagCount)
                } catch {
                    Issue.record(error)
